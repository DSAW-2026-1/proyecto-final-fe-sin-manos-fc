...
export default function AdminLogin() {
  ...
  const validate = () => {
    ...
    else if (!email.endsWith('@admin.unisabana.edu.co')) e.email = 'Debe usar un correo de administrador'
  ...
  const handleSubmit = async () => {
    ...
    setLoading(false)
    if (!res.ok) {
      if (res.status === 401) setApiError('Credenciales incorrectas.')
      else if (res.status === 403) setApiError('Cuenta suspendida o sin permisos de administrador.')
      ...
      <div className="animate-fadeUp" style={{ width: '100%', maxWidth: 400, position: 'relative' }}>
        ...
        <div style={{ background: 'var(--white)', borderRadius: 'var(--radius-xl)', padding: '36px 32px', boxShadow: '0 24px 60px rgba(0,0,0,0.4)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
            <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 24, fontWeight: 700, color: 'var(--navy)' }}>Acceso Administrador</h2>
            <span style={{ fontSize: 10, fontWeight: 700, padding: '3px 8px', borderRadius: 100, background: '#FDECEA', color: 'var(--danger)', border: '1px solid #F5C6C2', whiteSpace: 'nowrap' }}>Admin</span>
          </div>
          <p style={{ fontSize: 13, color: 'var(--gray-400)', marginBottom: 28 }}>Panel de control — acceso restringido</p>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
            ...
            <div className="input-group">
              ...
              <input className="input-field" type="email" placeholder="correo@admin.unisabana.edu.co"
                value={email} onChange={e => { setEmail(e.target.value); setApiError(''); setErrors(p => ({ ...p, email: '' })) }}
            ...
            <div className="input-group">
              ...
              <input className="input-field" type="password" placeholder="••••••••"
                value={password} onChange={e => { setPassword(e.target.value); setApiError(''); setErrors(p => ({ ...p, password: '' })) }}
                onKeyDown={e => e.key === 'Enter' && handleSubmit()} />
              {errors.password && <span className="input-error">{errors.password}</span>}
            </div>
            <button onClick={handleSubmit} disabled={loading} style={{
              width: '100%', padding: 14, background: 'var(--navy-dark)', color: 'var(--white)',
            ...
            <div style={{ borderTop: '1px solid var(--gray-100)', paddingTop: 18, textAlign: 'center' }}>
              <span style={{ fontSize: 13, color: 'var(--gray-400)' }}>¿Eres estudiante? </span>
              <button onClick={() => navigate('/login')} style={{ background: 'none', border: 'none', color: 'var(--navy)', fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: 'var(--font-body)' }}>Inicia sesión aquí</button>
            </div>
          </div>
        </div>
      </div>
